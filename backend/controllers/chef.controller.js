  ...
  try {
    ...
    for (let item of availability) {
      if (!daysOfWeek.includes(item.day)) {
        return res.status(400).json({ error: `Invalid day: ${item.day}` });
      }
    }
    ...
    for (let item of availability) {
      ...
      if (!start.isValid() || !end.isValid()) {
        return res
          .status(400)
          .json({ error: `Invalid time format in ${item.day}` });
      }
      ...
      if (duration <= 0) {
        return res
          .status(400)
          .json({ error: `End time must be after start time on ${item.day}` });
      }

      totalHours += duration;
    }

    // Multiply total hours by price
    const totalEarning = totalHours * price;
...
exports.getChefAvailabilityById = async (req, res) => {
  try {
    const chefAvailability = await ChefAvailability.find({
    ...
    if (!chefAvailability.length) {
      return res
        .status(404)
        .json({ message: "No availability found for this chef" });
    }

    // res.json(chefAvailability);
    return res.status(200).send({
      data: chefAvailability,
      message: "Chef Availability get successfully",
      status: 200,
    });
  } catch (error) {
...
exports.updateChefAvailability = async (req, res) => {
  ...
  try {
    ...
    if (!updatedAvailability) {
      return res
        .status(404)
        .json({ message: "Availability not found", status: 404 });
    }
...
exports.searchChefs = async (req, res) => {
  try {
    ...
    if (chefAvailability.length === 0) {
      return res
        .status(200)
        .json({ data: [], message: "No chefs found", status: 200 });
    }
...
exports.bookChef = async (req, res) => {
  try {
    ...
    const chefAvailability = await ChefAvailability.findOne({
      _id: chefAvailabilityId,
      chefId,
    });
    ...
    if (!chefAvailability) {
      return res
        .status(404)
        .json({ message: "Chef is not available for booking", status: 404 });
    }
    // Fetch user details
    ...
    if (existingBooking) {
      return res.status(400).json({
        message: "You have already booked this chef for this time slot.",
        status: 400,
      });
    }
    ...
    res.status(200).json({
      booking: newBooking,
      message: "Chef booked successfully",
      status: 200,
    });
  } catch (error) {
    res.status(500).json({ message: error.message, status: 500 });
...
exports.getChefBookings = async (req, res) => {
  try {
    ...
    if (!bookings.length) {
      return res
        .status(404)
        .json({ message: "No bookings found for this chef", status: 404 });
    }
...
exports.getChefReviews = async (req, res) => {
  try {
    ...
    if (!reviews.length) {
      return res
        .status(404)
        .json({ message: "No reviews found for this chef", status: 404 });
    }

    res.status(200).json({
      data: reviews,
      message: "Chef reviews get successfully",
      status: 200,
    });
  } catch (error) {
    res.status(500).json({ message: error.message, status: 500 });
...
exports.getChefNotification = async (req, res) => {
  try {
    ...
    if (!alrt.length) {
      return res
        .status(404)
        .json({ message: "No notification found for this chef", status: 404 });
    }

    res.status(200).json({
      data: alrt,
      message: "Chef notification get successfully",
      status: 200,
    });
  } catch (error) {
    res.status(500).json({ message: error.message, status: 500 });
